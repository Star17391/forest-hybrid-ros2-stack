...
public:
  ...
  : rclcpp::Node("hybrid_hop_executor")
  {
    ...
    landed_z_ = declare_parameter<double>("landed_z_m", 0.5);
...
private:
  ...
  bool at_xy(double tx, double ty, double tol) const
  {
    return std::hypot(tx - x_, ty - y_) <= tol;
  }

  bool at_aerial() const
  {
    return at_xy(land_x_, land_y_, aerial_xy_tol_) &&
           std::abs(cruise_alt_ - z_) <= aerial_z_tol_;
  }
  ...
  void tick()
  {
    ...
    switch (phase_) {
      case Phase::WAIT_AERIAL: {
        if (fsm_state_ == "AERIAL_READY" && !armed_started_ && !arming_in_progress_.load()) {
          armed_started_ = true;
        ...
        } else if (airborne_ && (fsm_state_ == "AERIAL_FLY" || fsm_state_ == "AERIAL_HOVER")) {
      ...
      case Phase::LANDING: {
        // ArduPilot (modo LAND) controla a descida com as hélices verticais.
        if (z_ <= landed_z_) {
